    ...
    for npz_file in tqdm(sorted(npz_dir.glob("*.npz"))):
        try:
            ...
            for signal, label in zip(signals, labels):
                spec = adaptive_superlet_transform(signal, freqs=freqs, sampling_freq=fs,
                ...
                spec = resize_inst(spec)  # Resize to (1, 30, 100)
                spec = normalize_inst(spec)  # Normalize
...
if __name__ == "__main__":
    convert_npz_to_superlet(
        npz_dir=Path("/home/jycha/ICTC/processed_npz"),
        save_dir=Path("/home/jycha/ICTC/superlet_transformed")
    )